...
class _InventoryScreenState extends State<InventoryScreen> {
  ...
  Future<String?> chooseMaster({required String type, String? current}) async {
    ...
    return showModalBottomSheet<String>(
      ...
      builder: (sheetContext) => StatefulBuilder(builder: (sheetContext, setLocal) {
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (_, index) {
                      ...
                      return ListTile(
                        leading:
                            Icon(option.favorite ? Icons.star : Icons.label_outline),
                        title: Text(option.value),
                        subtitle: Text(option.category),
                        trailing:
                            current == option.value ? const Icon(Icons.check) : null,
  ...
  Future<String?> _quickAddMaster(String type) async {
    ...
    final category =
        TextEditingController(text: type == 'texture' ? 'Finish' : 'General');
    bool favorite = false;
            ...
            children: [
              TextField(
                ...
                decoration: InputDecoration(
                  labelText:
                      type == 'size' ? 'Size (example: 12×18)' : 'Finish name',
                ),
              ),
              TextField(
                controller: category,
          ...
          actions: [
            ...
            FilledButton(
              onPressed: () async {
                ...
                try {
                  await LocalDb.instance.saveMasterOption(
                    MasterOption(
                      type: type,
                      value: text,
            ...
            child: Column(
              ...
              children: [
                ...
                ListTile(
                  ...
                  onTap: () async {
                    final sel = await chooseMaster(type: 'size', current: size);
                    if (sel != null) setLocal(() => size = sel);
                  },
                ),
                ListTile(
                  contentPadding: EdgeInsets.zero,
                  ...
                  trailing: const Icon(Icons.arrow_drop_down),
                  onTap: () async {
                    final sel = await chooseMaster(type: 'texture', current: texture);
                    if (sel != null) setLocal(() => texture = sel);
                  },
                ),
                TextField(
                  controller: stock,
              ...
              onPressed: () async {
                if (name.text.trim().isEmpty || size.isEmpty || texture.isEmpty) {
                  ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Enter tile name, size and finish.')));
                  return;
                }
                final uid = item?.clientUid ?? const Uuid().v4();
                var persistent = item?.localImage ?? '';
                if (imageChanged || (item == null && imagePath.isNotEmpty)) {
                  persistent = await ImageStorageService.persistPickedImage(
  ...
  Future<void> deleteItem(InventoryItem item) async {
    final ok = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Delete ${item.tileName}? Existing sales linked to this item will not be deleted.'),
        actions: [
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        floatingActionButton:
            FloatingActionButton.extended(onPressed: () => itemDialog(), icon: const Icon(Icons.add), label: const Text('Add Tile')),
        body: loading
...
class _InventoryImage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (file != null && file.existsSync()) {
      return ClipRRect(borderRadius: BorderRadius.circular(8), child: Image.file(file, width: 58, height: 58, fit: BoxFit.cover));
    }
